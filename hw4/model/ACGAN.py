...
class generator(nn.Module):
    ...
    def __init__(self, input_dim=62+13):
...
class ACGAN(object):
    def __init__(self, model_name='ACGAN', path='./output/',
                 sample_num=3, mu=0, sigma=1,
                 z_dim=62, c_dim=13, g_early_stop=None, n_loop_G=3, n_loop_D=1):
        # parameters
        ...
        self.c_dim = c_dim # label dimension
        
        # networks init
        self.G = generator(input_dim=z_dim+c_dim)
        ...
        if self.gpu_mode:
            ...
            self.BCE_loss = nn.BCELoss() # img loss
            self.MSE_loss = nn.MSELoss() # labels loss_func
        ...
        self.sample_y = self.sample_labels(num=sample_num)
    ...
    def train(self, dataloader, n_epoch=1000, g_early_stop=None, n_loop_G=3, n_loop_D=1):
        ...
        for epoch in range(n_epoch):
            ...
            for itr, (x_, y_vec_) in enumerate(dataloader):
                ...
                for _ in range(n_loop_D):
                    ...
                    C_real_loss = self.MSE_loss(C_real, y_vec_)
                    # fake
                    G_ = self.G(z_, y_vec_)
                    D_fake, C_fake = self.D(G_)
                    D_fake_loss = self.BCE_loss(D_fake, self.y_fake_)
                    C_fake_loss = self.MSE_loss(C_fake, y_vec_)
                ...
                for _ in range(n_loop_G):
                    ...
                    G_loss += self.MSE_loss(C_fake, y_vec_)
                    
                    mean_G_loss += G_loss.data[0]
                    
                    G_loss.backward()
                    self.G_optimizer.step()
                if n_loop_G is not 0:
            ...
            if (epoch+1) % 10 ==0:
                acgan.save(class_='_d2g1_'+str(epoch+101))
...
#         image_frame_dim = int(np.floor(np.sqrt(tot_num_samples)))
        ...
        if fix:
            ...
            sample_y = self.sample_labels(num=num)
...
#                           self.result_dir + '/' + self.dataset + '/' + self.model_name + '/' + self.model_name + '_epoch%03d' % epoch + '.png')
    ...
    def sample_labels(self, num=3):
        sample_y = np.random.rand(num, self.c_dim)
        
        if self.gpu_mode:
            sample_y = Variable(torch.FloatTensor(sample_y).cuda(), volatile=True)
        else:
            sample_y = Variable(torch.FloatTensor(sample_y), volatile=True)
        return sample_y
    ...
    def load(self, path=None, class_=''):
        ...
        print('successfully load these weights')
